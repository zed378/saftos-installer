...
const (
	...
	statusSettingUpHarv = "Setting up SaftOS"
...
func footer(g *gocui.Gui) error {
	...
	if v, err := g.SetView("footer", 0, maxY-2, maxX, maxY); err != nil {
		...
		v.Frame = false
		fmt.Fprintf(v, "<Use F12 to switch between SaftOS console and Shell>")
	}
	return nil
}
...
func (c *Console) getHarvesterConfig() error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			logrus.Infof("no existing saftos config detected in %s", defaultHarvesterConfig)
			return nil
		}
