...
func addDiskPanel(c *Console) error {
	...
	wipeDisksTitlePanelV.SetContent("Additional SaftOS installations detected")
...
func addAskCreatePanel(c *Console) error {
	askOptionsFunc := func() ([]widgets.Option, error) {
		options := []widgets.Option{
			{
				...
				Text:  "Create a new SaftOS cluster",
			}, {
				Value: config.ModeJoin,
				Text:  "Join an existing SaftOS cluster",
			},
		}
		...
		if !alreadyInstalled {
			options = append(options, widgets.Option{
				...
				Text:  "Install SaftOS binaries only",
			})
		}
		return options, nil
	}
	...
	askCreateV.PreShow = func() error {
		...
		if alreadyInstalled {
			return c.setContentByName(titlePanel, "SaftOS already installed. Choose configuration mode")
		}
...
func addCloudInitPanel(c *Console) error {
	...
	cloudInitV.PreShow = func() error {
		...
		return c.setContentByName(titlePanel, "Optional: remote SaftOS config")
	}
	gotoNextPage := func() error {
		cloudInitV.Close()
		...
		if !c.config.Install.Silent {
			if alreadyInstalled {
				confirmV.SetContent(options +
					"\nSaftOS is already installed. It will be configured with the above configuration. Continue?\n")
			} else if installModeOnly {
				confirmV.SetContent(options +
					"\nSaftOS will be copied to local disk. No configuration will be performed. Continue?\n")
			} else {
				confirmV.SetContent(options +
					"\nYour disk will be formatted and SaftOS will be installed with the above configuration. Continue?\n")
			}
		}
		c.Gui.Cursor = false
...
func addConfirmUpgradePanel(c *Console) error {
	...
	confirmV.PreShow = func() error {
		return c.setContentByName(titlePanel, fmt.Sprintf("Confirm upgrading SaftOS to %s?", version.Version))
	}
	confirmV.KeyBindings = map[gocui.Key]func(*gocui.Gui, *gocui.View) error{
		gocui.KeyEnter: func(_ *gocui.Gui, _ *gocui.View) error {
			confirmed, err := confirmV.GetData()
...
func addInstallPanel(c *Console) error {
	...
	installV.Title = " Installing SaftOS "
...
func addUpgradePanel(c *Console) error {
	...
	upgradeV.Title = " Upgrading SaftOS "
